...
export default {
  ...
  methods: {
    handleSubmit() {
      ...
      // إرسال البيانات إلى API الباك اند
      fetch("http://localhost/copstone4-mvc3/public/api/v1/employees", {
  method: "POST",
  ...
  body: JSON.stringify(this.employee)
      ...
      .then(data => {
        if (data.error) {
          ...
          // إنشاء الموظف الجديد مع ID من الباك
      ...
      .catch(err => {
        console.error(err);
        alert("حدث خطأ أثناء الإضافة. تأكد من أن الباك API يعمل ويعيد JSON صالح.");
      });
    },
...
export default {
  ...
  methods: {
    ...
    fetch("http://localhost/copstone4-mvc3/public/api/employees/add", {
  ...
  methods: {
    async handleSubmit() {
      try {
        ...
        const empId = data.data.id;
        alert(data.data.message || "تمت إضافة الموظف");
